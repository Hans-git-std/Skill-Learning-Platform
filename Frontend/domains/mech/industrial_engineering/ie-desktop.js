...
class IeDesktopSimulators {
    ...
    initCh2() {
        const desktop = this.setupCanvas('sim-ch2-desktop', 600);
        if(!desktop) return;
        ...
        const draw = (sim) => {
            ...
            let stations = [
                { x: 100, y: 300, time: parseInt(st1?.value || 5), color: '#0ea5e9', busy: false },
                { x: 300, y: 300, time: parseInt(st2?.value || 8), color: '#10b981', busy: false },
                { x: 500, y: 300, time: parseInt(st3?.value || 4), color: '#f59e0b', busy: false },
                { x: 700, y: 300, time: parseInt(st4?.value || 6), color: '#8b5cf6', busy: false }
            ];
            ...
            items = items.filter(item => item.pos < canvas.width + 100);
            ...
            stations.forEach((st, i) => {
                ...
                ctx.beginPath(); ctx.roundRect(st.x - 40, st.y - 60, 80, 60, 8); ctx.fill();
                ctx.fillStyle = '#fff'; ctx.font = '16px monospace'; ctx.textAlign = 'center';
                ctx.fillText(`ST ${i+1}`, st.x, st.y - 35);
                ctx.fillText(`${st.time}s`, st.x, st.y - 15);
                if(st.busy) {
                    ctx.fillStyle = '#ef4444'; ctx.beginPath(); ctx.arc(st.x-30, st.y-50, 5, 0, Math.PI*2); ctx.fill();
                }
            });
            
            ctx.fillStyle = '#f97316';
            items.forEach(item => {
                ctx.fillRect(item.pos - 10, 320, 20, 20);
            });
            ...
            ctx.fillText(`Cycle: ${cycleTime}s | Eff: ${efficiency.toFixed(1)}%`, 50, 50);
        };
    ...
    initCh3() {
        const desktopContainer = document.getElementById('sim-ch3-desktop');
        ...
        let scene, camera, renderer, controls, base, arm, ui;
        const check3D = () => {
            ...
            try {
                ...
                base = new THREE.Mesh(new THREE.CylinderGeometry(0.5, 0.5, 0.5, 32), new THREE.MeshPhongMaterial({ color: 0x3f3f46 }));
                arm = new THREE.Mesh(new THREE.BoxGeometry(0.2, 3, 0.2), new THREE.MeshPhongMaterial({ color: 0xf97316 }));
                arm.position.y = 1.5;
                base.add(arm);
                scene.add(base);

                ui = document.createElement('div');
                ui.style.position = 'absolute'; ui.style.bottom = '20px'; ui.style.left = '20px';
                ui.style.color = '#0ea5e9'; ui.style.fontFamily = 'monospace'; ui.style.fontSize = '24px';
                desktopContainer.appendChild(ui);
            } catch (e) {
                console.warn("WebGL not supported for desktop view", e);
            }
        };
        ...
        const animateDesktop = () => {
            ...
            if (desktopContainer.clientWidth > 0 && scene) {
                if (isRunning) {
                    arm.rotation.z = Math.sin(t * 2) * 0.5; 
                    base.rotation.y = Math.cos(t) * 0.5;
                }
                ui.innerHTML = `Stopwatch: ${t.toFixed(2)}s`;
                controls.update();
    ...
    initCh4() {
        const desktop = this.setupCanvas('sim-ch4-desktop', 600);
        if(!desktop) return;
        ...
        const draw = (sim) => {
            ...
            nodes.forEach(n => {
                ...
                ctx.fillStyle = '#27272a';
                ctx.beginPath(); ctx.arc(ax, y, 40, 0, Math.PI*2); ctx.fill();
    ...
    initCh6() {
        const desktop = this.setupCanvas('sim-ch6-desktop', 500);
        if(!desktop) return;
    ...
    initCh7() {
        const desktop = this.setupCanvas('sim-ch7-desktop', 500);
        if(!desktop) return;
    ...
    initCh8() {
        const desktop = this.setupCanvas('sim-ch8-desktop', 500);
        if(!desktop) return;
        ...
        const draw = (sim) => {
            ...
            const ox = 50, oy = canvas.height - 50, scale = 25;
    ...
    initCh12() {
        let desktopContainer = document.getElementById('sim-ch12-desktop');
        if(!desktopContainer) return;
        
        const angleInput = document.getElementById('ch12-angle');
        const weightInput = document.getElementById('ch12-weight');
        
        let scene, camera, renderer, controls, spine, box, ui;
        const check3D = () => {
            if (scene || desktopContainer.clientWidth === 0) return;
            try {
                scene = new THREE.Scene();
                ...
                spine = new THREE.Mesh(new THREE.CylinderGeometry(0.1, 0.1, 2, 16), new THREE.MeshPhongMaterial({color: 0x0ea5e9}));
                spine.position.y = 1;
                scene.add(spine);
                
                box = new THREE.Mesh(new THREE.BoxGeometry(0.5, 0.5, 0.5), new THREE.MeshPhongMaterial({color: 0xf97316}));
                box.position.set(1, 0.25, 0);
                scene.add(box);

                scene.add(new THREE.GridHelper(10, 10, 0x444444, 0x222222));

                ui = document.createElement('div');
                ui.style.position = 'absolute'; ui.style.top = '20px'; ui.style.right = '20px';
        ...
        const animateDesktop = () => {
            if (desktopContainer.clientWidth > 0 && scene) {
                ...
                box.scale.setScalar(weight / 10); 
                box.position.set(1 + Math.sin(angle)*2, 0.25, 0); 
                
                const rwl = 23 * (25 / (25 + angle * 50)); 
                ui.innerHTML = `Spine Flexion: ${(angle * 180 / Math.PI).toFixed(0)}°<br>Rec. Limit: ${rwl.toFixed(1)} kg<br>Actual: ${weight} kg`;
                ui.style.color = weight > rwl ? '#ef4444' : '#10b981';
